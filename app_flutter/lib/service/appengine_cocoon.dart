...
class AppEngineCocoonService implements CocoonService {
  ...
  Future<CocoonResponse<bool>> fetchTreeBuildStatus({
    ...
    return CocoonResponse<bool>.data(
        jsonResponse['AnticipatedBuildStatus'] == 'Succeeded');
  }
  ...
  Future<CocoonResponse<List<Agent>>> fetchAgentStatuses() async {
    ...
    if (response.statusCode != HttpStatus.ok) {
      return CocoonResponse<List<Agent>>.error(
          '/api/public/get-status returned ${response.statusCode}');
    }

    try {
      final Map<String, Object> jsonResponse = jsonDecode(response.body);
      return CocoonResponse<List<Agent>>.data(
          _agentStatusesFromJson(jsonResponse['AgentStatuses']));
    } catch (error) {
  ...
  @override
  Future<bool> rerunTask(Task task, String idToken, String commitSha) async {
    assert(idToken != null);
    ...
    final http.Response response = await _client.post(postResetTaskUrl,
        ...
        body: jsonEncode(<String, String>{
          if (qualifiedTask.isDevicelab) 'Key': task.key.child.name,
          if (qualifiedTask.isLuci) ...<String, Object>{
            'LuciBuilderName': task.name,
            'CommitSha': commitSha,
          }
        }));
