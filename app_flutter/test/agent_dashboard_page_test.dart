...
void main() {
  testWidgets('shows sign in button', (WidgetTester tester) async {
    await tester.pumpWidget(
        const MaterialApp(home: FakeInserter(child: AgentDashboardPage())));
  ...
  testWidgets('create agent FAB opens dialog', (WidgetTester tester) async {
    ...
    expect(find.text('CREATE AGENT'),
        findsOneWidget); // the floating action button
  ...
  testWidgets('create agent dialog calls create agent',
    ...
    when(mockCocoonService.fetchAgentStatuses())
        .thenAnswer((_) => Completer<CocoonResponse<List<Agent>>>().future);
        ...
        List<Agent>.generate(
          ...
          (int i) => Agent()
            ...
            ..healthCheckTimestamp =
                Int64(DateTime.utc(2000, 1, 1, i).millisecondsSinceEpoch)
