...
void main() {
  ...
  test(
    'findPrCheckRunsForSha consolidates multiple documents for the same SHA',
    () async {
      const sha = '9969edcc401f9ec849592fac1fc62ab09bb905a8';
      final pr = generatePullRequest(id: 1234, headSha: sha);

      // First document: has only "Dashboard Checks"
      firestoreService.putDocument(
        Document(
          fields: {
            PrCheckRuns.kPullRequestField: Value(
            ...
            PrCheckRuns.kSlugField: Value(
              stringValue: json.encode(
                RepositorySlug('flutter', 'packages').toJson(),
              ),
            ),
            PrCheckRuns.kShaField: sha.toValue(),
            'Dashboard Checks': '1'.toValue(),
          },
          name: firestoreService.resolveDocumentName(
            PrCheckRuns.kCollectionId,
            'doc1',
      ...
      // Second document: has actual tests
      firestoreService.putDocument(
        Document(
          fields: {
            PrCheckRuns.kPullRequestField: Value(
            ...
            PrCheckRuns.kShaField: sha.toValue(),
            'Linux analyze master': '2'.toValue(),
          },
          name: firestoreService.resolveDocumentName(
            PrCheckRuns.kCollectionId,
            'doc2',
      ...
      final result = await PrCheckRuns.findPrCheckRunsForSha(
        firestoreService,
        sha,
      );
      expect(result, isNotNull);
      expect(result!.checkRuns, {
        'Dashboard Checks': '1',
        'Linux analyze master': '2',
      });
    },
  );
}
