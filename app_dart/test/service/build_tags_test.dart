...
void main() {
  group('BuildTags', () {
    ...
    test('add if absent does not add if existing', () {
      ...
      expect(set.buildTags, unorderedEquals([TriggerTypeBuildTag.autoRetry]));
    });
  });
  ...
  group('UserAgentBuildTag', () {
    test('can be produced', () {
      ...
      expect(
        ...
        bbv2.StringPair(key: 'user_agent', value: 'flutter-foo'),
      );
    });
  ...
  group('BuildSetBuildTag', () {
    group('ByPresubmitCommitBuildSetBuildTag', () {
      ...
      test('fallsback to UnknownBuildTag if parsing fails', () {
        final tag = BuildTag.from(
          bbv2.StringPair(key: 'buildset', value: 'sha/git\\/malformed'),
        );
        expect(
          tag,
          UnknownBuildTag(key: 'buildset', value: 'sha/git\\/malformed'),
        );
      });
    });
    ...
    group('ByPostsubmitCommitBuildSetBuildTag', () {
      test('can be produced', () {
        expect(
          ByPostsubmitCommitBuildSetBuildTag(
            commitSha: 'abc123',
          ).toStringPair(),
      ...
      test('fallsback to UnknownBuildTag if parsing fails', () {
        final tag = BuildTag.from(
          bbv2.StringPair(key: 'buildset', value: 'commit/git\\/malformed'),
        );
        expect(
          tag,
          UnknownBuildTag(key: 'buildset', value: 'commit/git\\/malformed'),
        );
      });
    });
      ...
      test('can be produced', () {
        expect(
          ...
          bbv2.StringPair(
            key: 'buildset',
            value:
                'commit/gitiles/flutter.googlesource.com/mirrors/flutter/+/abc123',
          ),
        );
      });
      ...
      test('fallsback to UnknownBuildTag if parsing fails', () {
        final tag = BuildTag.from(
          bbv2.StringPair(
            key: 'buildset',
            value:
                'commit/gitiles/MALFORMED.googlesource.com/mirrors/flutter/+/abc123',
          ),
        );
        expect(
          tag,
  ...
  group('GitHubPullRequestBuildTag', () {
    ...
    test('fallsback to UnknownBuildTag if parsing fails', () {
      final tag = BuildTag.from(
        bbv2.StringPair(key: 'github_link', value: 'https:/\\/malformed/1234'),
      );
      expect(
        tag,
        UnknownBuildTag(key: 'github_link', value: 'https:/\\/malformed/1234'),
      );
    });
  });
  ...
  group('GitHubCheckRunIdBuildTag', () {
    test('can be produced', () {
      expect(
        ...
        bbv2.StringPair(key: 'github_checkrun', value: '1234'),
      );
    });
    ...
    test('can be parsed', () {
      ...
      expect(tag, GitHubCheckRunIdBuildTag(checkRunId: 1234));
    });

    test('fallsback to UnknownBuildTag if parsing fails', () {
      final tag = BuildTag.from(
        bbv2.StringPair(key: 'github_checkrun', value: 'a-notANumber'),
      );
      expect(
        tag,
        UnknownBuildTag(key: 'github_checkrun', value: 'a-notANumber'),
      );
    });
  });
  ...
  group('SchedulerJobIdBuildTag', () {
    test('can be produced', () {
      expect(
        ...
        bbv2.StringPair(key: 'scheduler_job_id', value: 'flutter/Linux_foo'),
      );
    });
    ...
    test('can be parsed', () {
      ...
      expect(tag, SchedulerJobIdBuildTag(targetName: 'Linux_foo'));
    });

    test('fallsback to UnknownBuildTag if parsing fails', () {
      final tag = BuildTag.from(
        bbv2.StringPair(key: 'scheduler_job_id', value: 'malformed\\Bar'),
      );
      expect(
        tag,
        UnknownBuildTag(key: 'scheduler_job_id', value: 'malformed\\Bar'),
      );
    });
  });
  ...
  group('CurrentAttemptBuildTag', () {
    ...
    test('can be parsed', () {
      ...
      expect(tag, CurrentAttemptBuildTag(attemptNumber: 1));
    });

    test('fallsback to UnknownBuildTag if parsing fails', () {
      final tag = BuildTag.from(
        bbv2.StringPair(key: 'current_attempt', value: 'notANumber'),
      );
      expect(tag, UnknownBuildTag(key: 'current_attempt', value: 'notANumber'));
    });

    test('fallsback to UnknownBuildTag if unexpected number', () {
      final tag = BuildTag.from(
        bbv2.StringPair(key: 'current_attempt', value: '0'),
      );
      expect(tag, UnknownBuildTag(key: 'current_attempt', value: '0'));
    });
  });
  ...
  group('CipdVersionBuildTag', () {
    test('can be produced', () {
      expect(
        CipdVersionBuildTag(
          const CipdVersion(branch: 'foo-bar'),
        ).toStringPair(),
        bbv2.StringPair(key: 'cipd_version', value: 'refs/heads/foo-bar'),
      );
    });
    ...
    test('can be parsed', () {
      ...
      expect(tag, CipdVersionBuildTag(const CipdVersion(branch: 'foo-bar')));
    });

    test('fallsback to UnknownBuildTag if parsing fails', () {
      final tag = BuildTag.from(
        bbv2.StringPair(key: 'cipd_version', value: 'refs\\malformed/foo-bar'),
      );
      expect(
        tag,
        UnknownBuildTag(key: 'cipd_version', value: 'refs\\malformed/foo-bar'),
      );
    });
  });
  ...
  group('InMergeQueueBuildTag', () {
    ...
    test('can be parsed', () {
      ...
      expect(tag, InMergeQueueBuildTag());
    });

    test('fallsback to UnknownBuildTag if parsing fails', () {
      final tag = BuildTag.from(
        bbv2.StringPair(key: 'in_merge_queue', value: 'false'),
      );
      expect(tag, UnknownBuildTag(key: 'in_merge_queue', value: 'false'));
    });
  });
  ...
  group('TriggerTypeBuildTag', () {
    ...
    test('fallsback to UnknownBuildTag if parsing fails', () {
      expect(
        BuildTag.from(
          bbv2.StringPair(key: 'trigger_type', value: 'not_a_thing'),
        ),
