...
void main() {
  group('GuardStatus.calculate', () {
    test('returns failed if there are failed builds', () {
      expect(
        GuardStatus.calculate(failedBuilds: 1, remainingBuilds: 0, totalBuilds: 10),
        GuardStatus.failed,
      );
    });
    ...
    test('returns succeeded if no failures and no remaining builds', () {
      expect(
        GuardStatus.calculate(failedBuilds: 0, remainingBuilds: 0, totalBuilds: 10),
    ...
    test('returns waitingForBackfill if all builds are remaining', () {
      expect(
        GuardStatus.calculate(failedBuilds: 0, remainingBuilds: 10, totalBuilds: 10),
    ...
    test('returns inProgress if some builds are done and no failures yet', () {
      expect(
        GuardStatus.calculate(failedBuilds: 0, remainingBuilds: 5, totalBuilds: 10),
