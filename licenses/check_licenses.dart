...
Future<void> run(List<String> arguments) async {
  String cocoonPath = path.join(Directory.current.path, '..');
...
Future<void> verifyNoMissingLicense(String workingDirectory, { bool checkMinimums = true }) async {
  ...
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'sh', overrideMinimumMatches ?? 1, '#!/bin/bash\n' + _generateLicense('# '));
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'bat', overrideMinimumMatches ?? 1, _generateLicense(':: '));
...
Future<void> _verifyNoMissingLicenseForExtension(String workingDirectory, String extension, int minimumMatches, String license, { bool trailingBlank = true }) async {
  ...
  for (final File file in _allFiles(workingDirectory, extension, minimumMatches: minimumMatches)) {
    ...
    if (!contents.startsWith(RegExp(licensePattern)))
  ...
  while (pending.isNotEmpty) {
    ...
    if (entity is File) {
      ...
      if (path.basename(entity.path) == 'gradlew.bat')
        continue;
      if (path.basename(entity.path) == 'AppDelegate.h')
        continue;
      if (path.basename(entity.path) == 'Runner-Bridging-Header.h')
        continue;
      if (path.basename(entity.path).endsWith('g.dart'))
        continue;
      if (path.basename(entity.path).endsWith('pb.dart'))
        continue;
      ...
      if (path.basename(entity.path) == 'build')
        continue;
      if (path.basename(entity.path) == 'ios')
        continue;
      if (path.basename(entity.path) == 'android')
        continue;
