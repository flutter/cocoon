...
sealed class DiscordChannels {
  ...
  static const Snowflake github2 = Snowflake.value(
    1116095786657267722,
  ); // this value is >2^53 and thus cannot be used in JS mode
}
...
String stripBoilerplate(String message, {bool inline = false}) {
  var current = message;
  for (final candidate in boilerplates) {
...
Future<void> sendDiscordMessage({
  ...
  assert(
    (embedTitle == null) == (embedDescription == null) &&
  ...
  final messageBuilder = MessageBuilder(
      content: content,
      embeds: <EmbedBuilder>[
        for (final String url in embeds) EmbedBuilder(imageUrl: url),
  ...
  try {
    final message = await discord.httpEndpoints.sendMessage(
      channel,
      messageBuilder,
    );
    if (emoji != null) {
