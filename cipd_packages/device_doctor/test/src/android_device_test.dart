...
void main() {
  ...
  group('AndroidRecoverDevice', () {
    ...
    test('successfully killed running processes', () async {
      ...
      ).thenAnswer((_) => Future.value(pkgCleanupProcess));
      when(
        processManager.start(
          <Object>['adb', 'reboot'],
          workingDirectory: anyNamed('workingDirectory'),
        ),
      ).thenAnswer((_) => Future.value(rebootProcess));

      final bool result = await device.cleanDevice(processManager: processManager);
      expect(result, true);
    });
  });
