...
void main() {
  group('runningProcessesOnWindows', () {
    ...
    test('when there is a single matched process', () async {
      ...
      when(processManager.runSync(
              <String>['powershell', 'Get-CimInstance', 'Win32_Process']))
          .thenAnswer((_) => ProcessResult(1, 0, output, 'def'));
      final processes =
          runningProcessesOnWindows('abc', processManager: processManager);
  ...
  group('killAllRunningProcessesOnWindows', () {
    ...
    test('when there are unkilled processes', () async {
      ...
      final result = await killAllRunningProcessesOnWindows('abc',
