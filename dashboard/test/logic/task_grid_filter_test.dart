...
void main() {
  void testDefault(TaskGridFilter filter) {
    ...
    expect(
        filter
            .matchesCommit(CommitStatus()..commit = (Commit()..author = 'joe')),
        true);
    expect(
  ...
  test('map constructor allows unused values', () {
    expect(TaskGridFilter.fromMap(<String, String>{'repo': 'flutter'}),
        TaskGridFilter());
  });

  test('map constructor result matches field setters', () {
    expect(TaskGridFilter.fromMap(<String, String>{}), TaskGridFilter());
    expect(TaskGridFilter.fromMap(<String, String>{'taskFilter': 'foo'}),
    ...
    expect(TaskGridFilter.fromMap(<String, String>{'hashFilter': 'foo'}),
        TaskGridFilter()..hashFilter = RegExp('foo'));
    expect(TaskGridFilter.fromMap(<String, String>{'showMac': 'false'}),
        TaskGridFilter()..showMac = false);
    expect(TaskGridFilter.fromMap(<String, String>{'showStaging': 'false'}),
        TaskGridFilter()..showStaging = false);
    expect(TaskGridFilter.fromMap(<String, String>{'showBringup': 'false'}),
  ...
  test('staging filter show all tasks', () {
    ...
    for (final TaskGridFilter filter in filters) {
      expect(
          filter.matchesTask(QualifiedTask.fromTask(
              Task()..builderName = 'Staging_build_linux task')),
          true);
      expect(
          filter.matchesTask(QualifiedTask.fromTask(
              Task()..builderName = 'staging_build_mac task')),
          true);
      expect(
          filter.matchesTask(QualifiedTask.fromTask(
              Task()..builderName = 'Linux_android task')),
          true);
      expect(
          filter.matchesTask(QualifiedTask.fromTask(
              Task()..builderName = 'linux_android task')),
  ...
  test('staging filter name matches', () {
    ...
    for (final TaskGridFilter filter in filters) {
      ...
      expect(
          filter.matchesTask(QualifiedTask.fromTask(
              Task()..builderName = 'Linux_android staging_build')),
          true);
      expect(
        filter.matchesTask(QualifiedTask.fromTask(
            Task()..builderName = 'linux_android_staging_build_linux task')),
        true,
      );
    }
  });
  ...
  test('matches task name simple substring', () {
    ...
    for (final TaskGridFilter filter in filters) {
      expect(
          filter
              .matchesTask(QualifiedTask.fromTask(Task()..builderName = 'foo')),
          true);
      expect(
          filter
              .matchesTask(QualifiedTask.fromTask(Task()..builderName = 'Foo')),
      ...
      expect(
          filter
              .matchesTask(QualifiedTask.fromTask(Task()..builderName = 'fo')),
          false);
    }
  });
  ...
  void testStage({
    ...
    final TaskGridFilter trueFilterMap =
        TaskGridFilter.fromMap(<String, String>{fieldName: 'true'});
    final TaskGridFilter falseFilterMap =
        TaskGridFilter.fromMap(<String, String>{fieldName: 'false'});
  ...
  test('matches ios and android filters logic', () {
    final TaskGridFilter iosMacFilter = TaskGridFilter.fromMap(
        <String, String>{'showMac': 'false', 'showiOS': 'true'});
    final TaskGridFilter macIosFilter = TaskGridFilter.fromMap(
        <String, String>{'showMac': 'true', 'showiOS': 'false'});
    final TaskGridFilter macIosBothTrueFilter = TaskGridFilter.fromMap(
        <String, String>{'showMac': 'true', 'showiOS': 'true'});

    final TaskGridFilter androidLinuxFilter = TaskGridFilter.fromMap(
        <String, String>{'showLinux': 'false', 'showAndroid': 'true'});
    final TaskGridFilter linuxAndroidFilter = TaskGridFilter.fromMap(
        <String, String>{'showLinux': 'true', 'showAndroid': 'false'});
    final TaskGridFilter linuxAndroidBothTrueFilter = TaskGridFilter.fromMap(
        <String, String>{'showLinux': 'true', 'showAndroid': 'true'});
    final TaskGridFilter androidFalseFilter =
        TaskGridFilter.fromMap(<String, String>{'showAndroid': 'false'});
    ...
    expect(
        iosMacFilter
            .matchesTask(QualifiedTask.fromTask(Task()..builderName = 'Mac')),
        false);
    expect(
    ...
    expect(
        androidLinuxFilter
            .matchesTask(QualifiedTask.fromTask(Task()..builderName = 'Linux')),
        false);
    expect(
  ...
  test('matches commit sha simple substring', () {
    ...
    for (final TaskGridFilter filter in filters) {
      expect(
          filter
              .matchesCommit(CommitStatus()..commit = (Commit()..sha = 'foo')),
          true);
      expect(
          filter.matchesCommit(
  ...
  test('matches commit sha regexp', () {
    ...
    for (final TaskGridFilter filter in filters) {
      expect(
          filter
              .matchesCommit(CommitStatus()..commit = (Commit()..sha = 'z bc')),
          true);
      expect(
          filter.matchesCommit(
